...
function Progress({setScreen}:{setScreen:(s:Screen)=>void}){
 ...
 const muscleHistory=workoutHistory.filter(h=>h.group===muscle)
 const bestByDate=new Map<string,number>()
 muscleHistory.forEach(h=>bestByDate.set(h.date,Math.max(h.weight,bestByDate.get(h.date)??0)))
 const perfPoints=[...bestByDate.entries()].sort((a,b)=>a[0].localeCompare(b[0])).slice(-10).map(([date,v])=>({d:fmtHistoryDate(date).slice(0,5),v}))
 const perfChangePct=perfPoints.length>=2&&perfPoints[0].v>0?(perfPoints.at(-1)!.v-perfPoints[0].v)/perfPoints[0].v*100:null
 const perfLabel=perfChangePct==null?'EN ATTENTE':perfChangePct>1?'EN PROGRÈS':perfChangePct<-1?'EN BAISSE':'STABLE'
 const weekAgo=Date.now()-7*86400000
 const volumeThisWeek=workoutHistory.filter(h=>h.group===muscle&&new Date(h.date).getTime()>=weekAgo).length
 const bestWeightEver=muscleHistory.length?Math.max(...muscleHistory.map(h=>h.weight)):null
 const sessionsLogged=bestByDate.size
 ...
 return <main className="v9Page v94Progress"><header className="v9TitleBar"><button onClick={()=>setScreen('today')}>‹</button><div><b>PROGRESS</b></div><button className="v93Dots">☷</button></header>
  ...
  {tab==='perf'&&<><section className="v94PerfHero"><small>PERFORMANCE EN SÈCHE</small><b>{perfLabel}</b>{perfChangePct!=null&&<strong>{perfChangePct>0?'+':''}{perfChangePct.toFixed(1).replace('.',',')} %</strong>}<p>{perfPoints.length>=2?'La force est maintenue malgré le déficit : bon signal pour la préservation musculaire.':'Enregistre des séries sur ce muscle pour voir ta progression réelle.'}</p></section><div className="v94MuscleChips">{muscleGroups.map(g=><button className={muscle===g?'active':''} onClick={()=>setMuscle(g)} key={g}>{g}</button>)}</div><section className="v9HistoryChart">{perfPoints.length?<ResponsiveContainer width="100%" height={210}><BarChart data={perfPoints}><Bar dataKey="v" fill="#8feef0" radius={[4,4,0,0]}/><YAxis hide domain={['dataMin - 5','dataMax + 5']}/><XAxis hide/></BarChart></ResponsiveContainer>:<p className="v94NoData">Pas encore de série enregistrée sur {muscle}.</p>}</section><section className="v94PerfStats"><span><small>VOLUME (7J)</small><b>{volumeThisWeek} série{volumeThisWeek>1?'s':''}</b></span><span><small>MEILLEUR POIDS</small><b>{bestWeightEver!=null?`${bestWeightEver} kg`:'—'}</b></span><span><small>SÉANCES</small><b>{sessionsLogged}</b></span></section></>}
...
export default function V9App(){
 ...
 return <div className="v9Shell"><div className="v9Phone">{page}{showNav&&<BottomNav screen={screen} setScreen={s=>{setScreen(s);if(s==='workout')setWorkoutView('session')}}/>}</div></div>
}
